...
export async function getCollection() {
    ...
    const [rows] = await pool.query("SELECT id, title, year, runtime, format, location, seen FROM collection ORDER BY title ASC");
    return rows;    
}
...
export async function addPhysicalEntry(given) {
    ...
    if(given["location"] !== "undefined")
    {
        insertInto.push("location");
        values.push(given["location"]);
    }
...
export async function updateEntry(given) {
    ...
    let q = `
        UPDATE collection
        SET title = ?, location = ?, year = ?, runtime = ?, format = ?, genre = ?, seen = ?
        WHERE id = ?
    `
    ...
    */
    ...
    await pool.query(q, [given["title"], given["location"], given["year"], 
